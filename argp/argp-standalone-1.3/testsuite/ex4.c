/* ex4.c: Argp example #4 - a program with somewhat more complicated options */
...
\vThis part of the documentation comes *after* the options;\
 note that the text is automatically filled, but it is possible\
...
parse_opt(int key, char *arg, struct argp_state *state)
{
  ...
  switch (key) {
    ...
    case ARGP_KEY_ARG:
       ...
       * that we are interested in, so we can just use
...
int main(int argc, char **argv)
{
  ...
  if (arguments.abort) {
      /* The glibc example used error (10, 0, "ABORTED"), but that is
	   * not portable. */
