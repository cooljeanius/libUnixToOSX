/* argp-fmtstream.c: Word-wrapping and line-truncating streams
...
/* Flush FS to its stream, and free it (but do NOT close the stream).  */
void
...
__argp_fmtstream_update(argp_fmtstream_t fs)
{
	...
	while (buf < fs->p) {
		...
		if ((fs->point_col == 0) && (fs->lmargin != 0)) {
			...
			if ((fs->p + pad) < fs->end) {
				...
				buf += pad; /* Do NOT bother searching them.  */
			} else {
		...
		if (fs->wmargin < 0) {
			...
			if (nl < fs->p) {
				memmove((buf + (r - (size_t)fs->point_col)), nl,
						(size_t)(fs->p - nl));
	...
	/* Remember that we have scanned as far as the end of the buffer.  */
...
__argp_fmtstream_ensure(struct argp_fmtstream *fs, size_t amount)
{
	if ((size_t)(fs->end - fs->p) < amount) {
		...
		wrote = (ssize_t)FWRITE_UNLOCKED(fs->buf, 1, (fs->p - fs->buf),
										 fs->stream);
...
__argp_fmtstream_printf(struct argp_fmtstream *fs, const char *fmt, ...)
{
	...
	do {
		...
		out = (size_t)__vsnprintf(fs->p, avail, fmt, args);
		va_end(args);
		if (out >= avail) {
			size_guess = (out + 1);
		}
...
# if !defined(__OPTIMIZE__) || defined(__NO_INLINE__) || defined(__clang__) || defined(__STRICT_ANSI__) || defined(_GL_EXTERN_INLINE_APPLE_BUG)
...
# endif /* !__OPTIMIZE__ || __NO_INLINE__ || __clang__ || __STRICT_ANSI__ || _GL_EXTERN_INLINE_APPLE_BUG */
