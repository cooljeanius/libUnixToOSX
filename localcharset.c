...
# include <unistd.h>
# if HAVE_LANGINFO_CODESET && defined(HAVE_LANGINFO_H)
#  include <langinfo.h>
# else
/* see comment below regarding use of the setlocale() function */
/* (and also my follow-up to said comment) */
#  if (defined(HAVE_LOCALE_H) && !defined(__DJGPP__) && !defined(__sun)) || 0
#   include <locale.h>
#  endif /* (HAVE_LOCALE_H && !__DJGPP__ && !__sun) || 0 */
# endif /* HAVE_LANGINFO_CODESET && HAVE_LANGINFO_H */
# ifdef __CYGWIN__
...
get_charset_aliases(void)
{
  ...
  if (cp == NULL) {
      ...
      /* Concatenate dir and base into freshly allocated file_name.  */
      {
        ...
        file_name = (char *)malloc(dir_len + (size_t)add_slash + base_len + 1);
        if (file_name != NULL) {
...
/* Determine the character encoding for the current locale, and canonicalize it
