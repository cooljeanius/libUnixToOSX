/* canonicalize.c: Return the canonical absolute name of a given file.
...
/* Return true if we have already seen the triple, <FILENAME, dev, ino>.
      ...
      if ((end - start) == 0) {
        ...
        /* nothing */;
          ...
          if ((dest + (end - start)) >= rname_limit) {
			  ...
			  size_t new_size;
			  new_size = (size_t)(rname_limit - rname);
          ...
          *dest = '\0';
          ...
          if (logical && (can_mode == CAN_MISSING)) {
              /* Avoid the stat in this case as it is inconsequential.
               * i.e. we are neither resolving symlinks or testing
               * component existence.  */
              st.st_mode = 0;
          ...
          if (S_ISLNK(st.st_mode)) {
               ...
               * since it is actually possible to encounter the same symlink
              ...
              buf = areadlink_with_size(rname, (size_t)st.st_size);
              if (!buf) {
		  ...
		  } /* end conditional on symlinks */
	  } /* end use of "if... else..." to compare 'start' and 'end' */
  } /* end for-loop */
