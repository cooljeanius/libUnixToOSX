/* progreloc.c: Provide relocatable programs.
...
# if (defined(ENABLE_RELOCATABLE) && ENABLE_RELOCATABLE) || defined(__APPLE__)
...
#   ifdef __linux__
	 ...
	 * enclosed in brackets, which we cannot use here.  */
	{
		...
		{
			char buf[(6 + 10 + 5)]; /* why not just 21? */
...
#   if HAVE_MACH_O_DYLD_H && HAVE__NSGETEXECUTABLEPATH
	...
	/* use 'IS_PATH_WITH_DIR' macro: */
	if (!IS_PATH_WITH_DIR(location)) {
		/* not sure if this is correct... */
		return NULL;
	}
#   endif /* HAVE_MACH_O_DYLD_H && HAVE__NSGETEXECUTABLEPATH */
...
#   endif /* NO_XMALLOC */
					if (p_len == 0) {
						/* An empty PATH element designates the current
						 * directory. */
...
char * _GL_ATTRIBUTE_PURE /* this was suggested by '-Wsuggest-attribute=pure' */
