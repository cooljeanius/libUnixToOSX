...
When the name is complete, finalize it.  Then, if the symbol exists
...
In case that is unclear, when we have enough chars to make up
...
struct obstack          /* control current object in current chunk */
{
  ...
  unsigned maybe_empty_object:1;/* There is a possibility that the current
                                 * chunk contains a 0-length object.  This
                                 * prevents freeing the chunk if we
...
/* There are also some in obstack_printf.c: */
...
/* Error handler called when 'obstack_chunk_alloc' failed to allocate
   ...
   should either abort gracefully or use longjump - but should NOT
...
/* Mask specifying low bits that should be clear in address of object: */
...

#if defined __GNUC__
/* NextStep 2.0 cc was really gcc 1.93, but it defined __GNUC__ = 2 and
 * failed to implement __extension__.  But that compiler did NOT define
 * __GNUC_MINOR__ though: */
 ...
 * all args only once without using a global variable.
...
({ struct obstack *__o = (OBSTACK);                                     \
   ...
   if (__o->next_free + __len > __o->chunk_limit)                       \
     _obstack_newchunk(__o, __len);                                     \
   memcpy(__o->next_free, where, (size_t)__len);                        \
   __o->next_free += __len;                                             \
   (void)0; })
 ...
 * so that we can avoid having void expressions in the arms of the
 * conditional expression.
 * Casting the third operand to void was tried before,
 * but some compilers fail to accept it: */
