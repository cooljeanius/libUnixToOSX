           ...
           int delim1, int delim2, FILE *stream)
{
  ...
  do {
      ...
      if (buffer) {
          if (delim1 != EOF) {
              const char *end;
			  end = (const char *)memchr2(buffer, delim1, delim2, buffer_len);
              if (end) {
      ...
      /* We always want at least one byte left in the buffer, since we always
	   * (unless we get an error while reading the first byte) NUL-terminate
	   * the line buffer. */
      if ((nbytes_avail < (buffer_len + 1)) && (size < nmax)) {
          ...
          if ((newsize - (size_t)(read_pos - ptr)) < (buffer_len + 1)) {
			  newsize = ((size_t)(read_pos - ptr) + buffer_len + 1);
		  }
          ...
          newptr = (char *)realloc(ptr, newsize);
          if (!newptr) {
			  goto unlock_done;
		  }
          ptr = newptr;
