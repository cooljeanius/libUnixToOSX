/* allocator.h: Memory allocators such as malloc+free.
 *
...
struct allocator
{
   ...
   * When given a zero size it may return NULL even if successful. */
  void *(*reallocate)(void *, size_t);

  /* Call FREE to free memory, like 'free'. */
  void (*free)(void *);

  /* If nonnull, call DIE(SIZE) if MALLOC(SIZE) or REALLOC(..., SIZE) fails.
   * DIE should not return. SIZE should equal SIZE_MAX if size_t overflow was
   * detected while calculating sizes to be passed to MALLOC or REALLOC. */
  void (*die)(size_t);
};
