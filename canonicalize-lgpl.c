/* canonicalize-lgpl.c: Return the canonical absolute name of a given file.
...
char *__realpath(const char *name, char *resolved)
{
  ...
  if (name[0] != '/') {
      if (!__getcwd(rpath, (size_t)path_max)) {
          rpath[0] = '\0';
          goto error;
	  }
      dest = strchr(rpath, '\0');
      ...
      if ((end - start) == 0) {
          ...
          *dest = '\0';
          ...
          if (S_ISLNK (st.st_mode)) {
              ...
              n = (int)__readlink(rpath, buf, (size_t)(path_max - 1));
              if (n < 0) {
              ...
              len = strlen(end);
              if ((long int)((size_t)n + len) >= path_max) {
                  freea(buf);
                  __set_errno(ENAMETOOLONG);
                  goto error;
			  }
              ...
              if (buf[0] == '/') {
                  dest = (rpath + 1);     /* It is an absolute symlink */
                  if (DOUBLE_SLASH_IS_DISTINCT_ROOT) {
                      if ((buf[1] == '/') && (buf[2] != '/')) {
						  *dest++ = '/';
					  }
