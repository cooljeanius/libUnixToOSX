...
# Copyright 1996, 1997, 1999, 2000, 2001, 2002  FSF, Inc.
...
# @defmac AM_REDONE_PROG_CC_STDC
# @maindex PROG_CC_STDC
# @ovindex CC
# If the C compiler in not in ANSI C mode by default, try to add an option
# to output variable @code{CC} to make it so.  This macro tries various
# options that select ANSI C on some system or another.  It considers the
# compiler to be in ANSI C mode if it handles function protos correctly.
#
# If you use this macro, you should check after calling it whether the C
# compiler has been set to accept ANSI C; if not, the shell variable
# @code{am_new_cv_prog_cc_stdc} is set to @samp{no}. If you wrote source
...
if test -z "${am_new_cv_prog_cc_stdc}"; then
  AC_MSG_RESULT([none needed])
else
  AC_MSG_RESULT([${am_new_cv_prog_cc_stdc}])
fi
case "x${am_new_cv_prog_cc_stdc}" in
  x|xno) ;;
