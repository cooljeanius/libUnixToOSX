...
AC_DEFUN([gl_FUNC_PREAD],[
  ...
  dnl# Persuade glibc <unistd.h> to declare pread() for us:
  AC_REQUIRE([AC_USE_SYSTEM_EXTENSIONS])dnl

  AC_CHECK_FUNCS_ONCE([pread])dnl
  ## if so:
  if test "x${ac_cv_func_pread}" = "xyes"; then
    dnl# On HP-UX 11.11 with _FILE_OFFSET_BITS=64, pread() on a file
    dnl# returns 0 instead of a positive value.
             ...
             ]],[[
{
  ...
  /* This test fails on HP-UX 11.00..11.11.  */
  {
    ...
    if (fd < 0)
      ...
      {
        char byte_buf;
        if (pread(fd, &byte_buf, 1, (off_t)0L) != 1)
          result |= 2;
        close(fd);
      }
  }
  /* This test fails on HP-UX 11.00..11.31.  */
  {
    ...
    if (fp == NULL)
    ...
    else
      {
        int fd = fileno(fp);
        char byte_buf;
        if (pread(fd, &byte_buf, 1, (off_t)0L) >= 0)
          result |= 4;
        pclose(fp);
      }
  }
