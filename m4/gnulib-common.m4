 ...
 * (unlike the GNU C semantics of earlier versions), but does not display
 * it by setting __GNUC_STDC_INLINE__.
...
# define _GL_ATTRIBUTE_CONST /* empty */
  ...
  dnl# Such diagnostics arise e.g., in the macros gl_PRINTF_DIRECTIVE_N,
  dnl# gl_SNPRINTF_DIRECTIVE_N.
  LIBC_FATAL_STDERR_=1
  export LIBC_FATAL_STDERR_
])dnl

# gl_MODULE_INDICATOR_CONDITION
# expands to a C preprocessor expression that evaluates to 1 or 0,
# depending whether a gnulib module that has been requested shall
# be considered present or not.
m4_define([gl_MODULE_INDICATOR_CONDITION],[1])dnl
...
AC_DEFUN([gl_MODULE_INDICATOR],[
  AC_DEFINE_UNQUOTED([GNULIB_]m4_translit([[$1]],
      ...
      [ABCDEFGHIJKLMNOPQRSTUVWXYZ___]),
    [gl_MODULE_INDICATOR_CONDITION],
    [Define to a C preprocessor expression (for $1) that evaluates
     to 1 or 0, depending on whether the gnulib module $1 shall
     be considered to be present (i.e. if we can use $1).])
])dnl
...
m4_ifndef([m4_foreach_w],
  [m4_define([m4_foreach_w],
    [m4_foreach([$1], m4_split(m4_normalize([$2]),[ ]),[$3])])])dnl
...
m4_ifndef([AS_VAR_IF],
[m4_define([AS_VAR_IF],
[AS_IF([test x"AS_VAR_GET([$1])" = x""$2],[$3],[$4])])])dnl
...
AC_DEFUN([gl_PROG_AR_RANLIB],[
  ...
  dnl# programs ar, ranlib produce libraries that work only with gcc,
  dnl# not with cc.
...
Amsterdam
        ...
        [gl_cv_c_amsterdam_compiler=no])dnl
    ])
  if test -z "${AR}"; then
    if test "x${gl_cv_c_amsterdam_compiler}" = "xyes"; then
      AR='cc -c.a'
...
# AC_C_RESTRICT
# This definition overrides the AC_C_RESTRICT macro from
# autoconf 2.60..2.61, so that mixed use of GNU C and GNU C++ and mixed
# use of Sun C and Sun C++ works.
...
# is like AC_CACHE_VAL(cache-id, command-to-set-it), except that it
