...
AC_DEFUN([gl_FUNC_PWRITE],[
  ...
  dnl# Persuade glibc <unistd.h> to declare pwrite() for us:
  AC_REQUIRE([AC_USE_SYSTEM_EXTENSIONS])dnl

  AC_CHECK_FUNCS_ONCE([pwrite])dnl
  ## if so...
  if test "x${ac_cv_func_pwrite}" = "xyes"; then
    dnl# On HP-UX 11.11 with _FILE_OFFSET_BITS=64, pwrite() on a file does
    dnl# not fail when an invalid (negative) offset is passed and uses an
    dnl# arbitrary offset instead of the argument.
             ...
             ]],[[
{
  ...
  return 0;
    ...
    ## decide to replace in case of failure:
