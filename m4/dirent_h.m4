...
AC_DEFUN([gl_DIRENT_H],[
  ...
  AC_REQUIRE([gl_DIRENT_H_DEFAULTS])dnl
  ...
  AC_SUBST([HAVE_DIRENT_H])dnl

  dnl# Check for declarations of anything we want to poison if the
  dnl# corresponding gnulib module is not in use.
  gl_WARN_ON_USE_PREPARE([[
#include <dirent.h>
    ]],[alphasort closedir dirfd fdopendir opendir readdir rewinddir scandir])dnl
])dnl

AC_DEFUN([gl_DIRENT_MODULE_INDICATOR],[
  dnl# Use AC_REQUIRE here, so that the defaults are expanded once only.
  AC_REQUIRE([gl_DIRENT_H_DEFAULTS])dnl
