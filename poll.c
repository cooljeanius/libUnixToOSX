...
# if defined __MACH__ && defined __APPLE__
      ...
      r = (int)recv(fd, NULL, (size_t)0, (int)MSG_PEEK);
...
# ifdef _SC_OPEN_MAX
  static int sc_open_max = -1;
	...
	/* TODO: not sure if my casting is correct here... */
  if (((int)nfd < 0)
	  || (((int)nfd > sc_open_max)
		  && ((sc_open_max != -1)
			  || ((int)nfd > (sc_open_max = (int)sysconf((int)_SC_OPEN_MAX)))))) {
      errno = EINVAL;
      return -1;
  }
# else /* !_SC_OPEN_MAX */
...
# endif /* !_SC_OPEN_MAX */
  ...
  /* EFAULT is not necessary to implement, but let us do it in the
   * simplest case. */
  if (!pfd && nfd) {
      errno = EFAULT;
      return -1;
